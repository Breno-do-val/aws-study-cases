...
- Termination Protection is _turned off_ by default, you must turn it on.
- On an EBS-backed instance, _the default action is for the root EBS volume to be deleted_ when the instance is terminated.
- EBS Root volumes of your Default AMI's _CAN_ be encrypted. You can also use a third party tool (such a bit locker ect) to encript the root volume, or this can be done when creating AMI's in the AWS console or using the API.
